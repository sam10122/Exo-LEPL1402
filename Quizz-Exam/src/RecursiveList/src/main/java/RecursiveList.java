...
public abstract class RecursiveList<A> implements Iterable<A> {
    ...
    public RecursiveList<A> reverse() {
        RecursiveList<A> copy = this;
        RecursiveList<A> other = RecursiveList.nil();
    ...
    public LinkedList<A> fills(RecursiveList<A> list, LinkedList<A> other){
        if(list.tail().isEmpty()){
            other.add(list.head());
            return other;
        }
        other.add(list.head());
        return fills(list.tail(),other);
    }
